...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `udf_get_meters_peaks` (`elevation` DECIMAL(19,4)) RETURNS VARCHAR(10) CHARSET utf8 COLLATE utf8_general_ci  BEGIN
DECLARE meters VARCHAR(10);
IF(elevation < 2000) THEN
	SET meters := 'Low';
ELSEIF(elevation >= 2000 AND elevation <= 5000) THEN
	SET meters := 'Medium';
...
CREATE DEFINER=`root`@`localhost` FUNCTION `udf_number_of_rivers_in_certain_country` (`country` VARCHAR(255)) RETURNS INT(11)  BEGIN
    	DECLARE result INT(11);
        
        SELECT COUNT(*) INTO result
		FROM countries_rivers, countries, rivers 
        WHERE countries_rivers.river_id = rivers.id AND countries_rivers.country_code = countries.country_code AND countries.country_name = country
        GROUP BY countries.country_name;
        
        RETURN result;
END$$

CREATE DEFINER=`root`@`localhost` FUNCTION `udf_show_longer_river` (`r1_id` INT, `r2_id` INT) RETURNS VARCHAR(255) CHARSET utf8 COLLATE utf8_general_ci  BEGIN
	DECLARE name1 VARCHAR(255);
    DECLARE name2 VARCHAR(255);
    DECLARE lengthN1 INT;
    DECLARE lengthN2 INT;
    
    SELECT river_name, length INTO name1, lengthN1
    FROM rivers
    WHERE rivers.id = r1_id;
    
    SELECT river_name, length INTO name2, lengthN2
    FROM rivers
    WHERE rivers.id = r2_id;
    
    IF(lengthN1 > lengthN2) THEN
    	RETURN CONCAT('The river ', name1, 'is longest of ', name2);
    ELSE
    	RETURN CONCAT('The river ', name2, 'is longest of ', name1);
    END IF;
...
--
-- Table structure for table `countries_rivers`
--
...
--
-- Dumping data for table `countries_rivers`
--
...
--
-- Table structure for table `mountains_countries`
--
...
--
-- Dumping data for table `mountains_countries`
--
...
ALTER TABLE `continents`
  ...
  ADD UNIQUE KEY `PK_Continents` (`continent_code`);
...
ALTER TABLE `countries`
  ...
  ADD UNIQUE KEY `PK_Countries` (`country_code`),
  ADD KEY `fk_countries_currencies` (`currency_code`),
  ADD KEY `fk_countries_continents` (`continent_code`);

--
-- Indexes for table `countries_rivers`
--
ALTER TABLE `countries_rivers`
  ADD PRIMARY KEY (`river_id`,`country_code`),
  ADD UNIQUE KEY `PK_CountriesRivers` (`country_code`,`river_id`);
...
ALTER TABLE `currencies`
  ...
  ADD UNIQUE KEY `PK_Currencies` (`currency_code`);
...
ALTER TABLE `mountains`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `PK_Mountains` (`id`);

--
-- Indexes for table `mountains_countries`
--
ALTER TABLE `mountains_countries`
  ADD PRIMARY KEY (`mountain_id`,`country_code`),
  ADD UNIQUE KEY `PK_MountainsContinents` (`mountain_id`,`country_code`),
  ADD KEY `fk_mountains_countries_mountains` (`country_code`);
...
ALTER TABLE `peaks`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `PK_Peaks` (`id`),
  ADD KEY `fk_peaks_mountains` (`mountain_id`);
...
--
ALTER TABLE `countries`
  ADD CONSTRAINT `fk_countries_continents` FOREIGN KEY (`continent_code`) REFERENCES `continents` (`continent_code`),
  ADD CONSTRAINT `fk_countries_currencies` FOREIGN KEY (`currency_code`) REFERENCES `currencies` (`currency_code`);

--
-- Constraints for table `countries_rivers`
--
ALTER TABLE `countries_rivers`
  ADD CONSTRAINT `fk_countries_rivers_countries` FOREIGN KEY (`country_code`) REFERENCES `countries` (`country_code`),
  ADD CONSTRAINT `fk_countries_rivers_rivers` FOREIGN KEY (`river_id`) REFERENCES `rivers` (`id`);

--
-- Constraints for table `mountains_countries`
--
ALTER TABLE `mountains_countries`
  ADD CONSTRAINT `fk_mountains_countries_countries` FOREIGN KEY (`mountain_id`) REFERENCES `mountains` (`id`),
  ADD CONSTRAINT `fk_mountains_countries_mountains` FOREIGN KEY (`country_code`) REFERENCES `countries` (`country_code`);

--
-- Constraints for table `peaks`
--
ALTER TABLE `peaks`
